...
db.serialize(() => {
  ...
  const insertUnidade = db.prepare('INSERT INTO unidades (id_local, identificacao, andar_ou_bloco, nome_morador, contato, dispositivo_instalado, device_id) VALUES (?, ?, ?, ?, ?, ?, ?)');
  ...
  nomesEdificios.forEach((nome, i) => {
    ...
    aptos.forEach(u => insertUnidade.run(u.id_local, u.identificacao, u.andar_ou_bloco, u.nome_morador, u.contato, u.dispositivo_instalado, u.device_id));
    localId++;
  });
  ...
  nomesCondominios.forEach((nome, i) => {
    ...
    casas.forEach(u => insertUnidade.run(u.id_local, u.identificacao, u.andar_ou_bloco, u.nome_morador, u.contato, u.dispositivo_instalado, u.device_id));
    localId++;
  });
  ...
  nomesResidencias.forEach((nome, i) => {
    ...
    casa.forEach(u => insertUnidade.run(u.id_local, u.identificacao, u.andar_ou_bloco, u.nome_morador, u.contato, u.dispositivo_instalado, u.device_id));
    localId++;
  });
